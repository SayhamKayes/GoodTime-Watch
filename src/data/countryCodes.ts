...
// Comprehensive ISO 3166-1 240+ Countries and Territories
export const COUNTRY_CODES: CountryCodeItem[] = [
  ...
  // All Global Countries Alphabetically
  ...
  { code: 'AZ', name: 'Azerbaizer', dialCode: '+994', flag: '🇦🇿' },
...
// Helper: Parse existing string (raw phone, tel: or wa.me) into { dialCode, nationalNumber }
export const parsePhoneNumberParts = (
  ...
  cleaned = cleaned.replace(/[^\d+]/g, ''); // keep digits and plus

  // Sort dial codes by length descending so +880 matches before +8, +971 before +9, etc.
  ...
  if (cleaned.startsWith('+')) {
    for (const item of sortedCodes) {
      if (cleaned.startsWith(item.dialCode)) {
        const national = cleaned.slice(item.dialCode.length);
        return { dialCode: item.dialCode, nationalNumber: national };
      }
    }
  }

  // If starts with digits without +, match against country codes with + stripped
  for (const item of sortedCodes) {
    const rawCode = item.dialCode.replace('+', '');
    if (cleaned.startsWith(rawCode) && cleaned.length > rawCode.length + 3) {
      const national = cleaned.slice(rawCode.length);
      return { dialCode: item.dialCode, nationalNumber: national };
    }
  }

  // Fallback: If starts with 01, it's typically local Bangladesh number
  if (cleaned.startsWith('01')) {
    return { dialCode: '+880', nationalNumber: cleaned };
  }

  return { dialCode: defaultDialCode, nationalNumber: cleaned };
};

// Clean national number when combining with country code (strips leading 0 if needed)
export const sanitizeForCountryCode = (nationalNumber: string): string => {
...
// Build formatted tel: URL (e.g. "tel:+8801327426905")
export const buildTelLink = (dialCode: string, nationalNumber: string): string => {
  const cleanCode = dialCode.startsWith('+') ? dialCode : `+${dialCode.replace(/\D/g, '')}`;
  const cleanNum = sanitizeForCountryCode(nationalNumber);
  if (!cleanNum) return '';
  return `tel:${cleanCode}${cleanNum}`;
};

// Build formatted wa.me URL (e.g. "https://wa.me/8801327426905")
export const buildWhatsAppLink = (dialCode: string, nationalNumber: string): string => {
  const codeDigits = dialCode.replace(/\D/g, '');
  const cleanNum = sanitizeForCountryCode(nationalNumber);
  if (!cleanNum) return '';
  return `https://wa.me/${codeDigits}${cleanNum}`;
};

// Build raw full WhatsApp digits (e.g. "8801327426905")
export const buildWhatsAppDigits = (dialCode: string, nationalNumber: string): string => {
  const codeDigits = dialCode.replace(/\D/g, '');
  const cleanNum = sanitizeForCountryCode(nationalNumber);
  if (!cleanNum) return '';
  return `${codeDigits}${cleanNum}`;
};

// Build international dial text (e.g. "+8801327426905")
export const buildInternationalDial = (dialCode: string, nationalNumber: string): string => {
  const cleanCode = dialCode.startsWith('+') ? dialCode : `+${dialCode.replace(/\D/g, '')}`;
  const cleanNum = sanitizeForCountryCode(nationalNumber);
  if (!cleanNum) return '';
